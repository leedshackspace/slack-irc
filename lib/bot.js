...
const ALLOWED_SUBTYPES = ['me_message', 'file_share', 'bot_message'];
...
class Bot {
  ...
  isFileUpload(message) {
    logger.debug(`Is this a file_share? : "${message.subtype}"`);
    return message.subtype == 'file_share'
  }

  mirrorUploadToImgur(message) {
    var private_url = message['file']['url_private'];
    ...
    delete fake_message.subtype;
    logger.debug(fake_message);
    logger.debug(channelName);
    var downloadFileName = tmp.tmpNameSync();
    var uploadFileName = downloadFileName;
    var downloadToken = this.options.token
    if (message.file.mimetype.substring(0,5) != 'image') {
      // Sneaky hack!  If we can't use imgur, just don't upload anywhere.
      uploadFileName = 'xyyzzy.nope';
      // also, don't download anywhere!
      downloadToken = 'NULL';
    }
    request
      .get(private_url)
      .auth(null,null,true,downloadToken)
      .on('response', function(response) {
        logger.debug(`HTTP response code from download : ${response.statusCode}`);
      })
      ...
      .on('finish', function() {
        imgur
          .uploadFile(uploadFileName)
        	.then(function(json) {
        	  fake_message.text = `Action: ${user.name} has just uploaded an image to slack, here it is on imgur : ${json.data.link}`;
            logger.debug('Just uploaded a file to imgur : ' + json.data.link);
        	})
          .catch(function(err) {
            logger.info('The imgur upload failed for some reason!  That reason was : ' + err.message);
            fake_message.text = `Hey @${user.name} the file you just uploaded to slack can't be mirrored to irc, fancy uploading it somewhere else?`;
            logger.debug(`Sending ${fake_message.text} to slack on channel mapped for ${ircChannel}.`)
            self.sendToSlack('MirrorImage',ircChannel,fake_message.text);
        	})
          .then(function() {
            logger.debug(`Sending ${fake_message.text} to irc.`);
            self.sendToIRC(fake_message);
            logger.debug('done.');
          });
        })
      );
  ...
  sendToIRC(message) {
    logger.debug(`Asked to send a message to IRC: "${message.text}"`);
    const { dataStore } = this.slack.rtm;
    const channel = dataStore.getChannelGroupOrDMById(message.channel);
    if (!channel) {
    ...
    if (message.bot_id == 'B42KVL3PF') { // CHANGEME
       logger.debug(`Not propagating our own echo: "${message.text}"`);
       return;
    }
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        const bot = dataStore.getBotById(message.bot_id);
        text = `[${bot.name}] ${text}`;
      } else if (this.isFileUpload(message)) {
        this.mirrorUploadToImgur(message);
        return; // we're not going to say anything until callbacks are resolved
      }
