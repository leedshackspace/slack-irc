...
class Bot {
  ...
  mirrorUploadToImgur(message) {
    ...
    request
      .get(private_url)
      .auth(null,null,true,this.options.token)
      ...
      .on('finish', function() {
        imgur
          ...
          .catch(function(err) {
            ...
            logger.debug(`Sending ${fake_message.text} to slack on channel ${channel.name}.`)
            this.sendToSlack('MirrorImage',channel.name,fake_message.text);
            logger.debug("Blind female stag.")
        	})
          .then(function() {
            logger.debug(`Sending ${fake_message.text} to irc.`)
            this.sendToIRC(fake_message);
          });
        })
      );
  }
