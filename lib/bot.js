      ...
      .on('finish', function() {
        imgur
          .uploadFile('doodle.jpg')
        	.then(function(json) {
        	  the_link = json.data.link;
            logger.info('Just uploaded a file to imgur : ' + the_link);
            console.log(the_link);
        	})
          .catch(function(err) {
            the_link = false;
            logger.error('The imgur upload failed for some reason!  That reason was : ' + err.message);
        	});
        });
      while (the_link === true) {
        //we wait for the_link to not be true - it should be a url or false
      }
      return the_link;
  }
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        var url = this.mirrorUploadToImgur(message);
        if (url == false) {
          text = `Hey @${user.name} the file you just uploaded to slack can't be mirrored to irc, fancy uploading it somewhere else?`;
        } else {
          text = `Action: ${user.name} has just uploaded an image to slack, here it is on imgur : ${url}`;
        }
      }
